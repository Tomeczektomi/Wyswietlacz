    ...
    setWyglad(Wyglad.STANDARDOWY);  
      ...
      public synchronized void paint(Graphics a) {
         ...
         for (int i = 0; i < liczbaWyswietlaczy; i++){ //rysowanie okreslonej liczby wyswietlaczy
           ...
           int ax [] = {(3+wyp)*rozm+(szer*rozm*i),(5+wyp)*rozm+(szer*rozm*i),(14-wyp)*rozm+(szer*rozm*i),(16-wyp)*rozm+(szer*rozm*i),(14-wyp)*rozm+(szer*rozm*i),(5+wyp)*rozm+(szer*rozm*i)};
           int ay [] = {2*rozm,0,0,2*rozm,4*rozm,4*rozm};
           a.setColor(colA);
           a.fillPolygon(ax,ay,npoints);
           
           int bx [] = {(17-wyp)*rozm+szer*rozm*i, (19-wyp)*rozm+szer*rozm*i, (19-wkl)*rozm+szer*rozm*i, (17-wkl)*rozm+szer*rozm*i, (15-wkl)*rozm+szer*rozm*i, (15-wyp)*rozm+szer*rozm*i};
           int by [] = {3*rozm, 5*rozm, 14*rozm, 16*rozm, 14*rozm, 5*rozm};
           a.setColor(colB);
           a.fillPolygon(bx,by,npoints);

           int cx [] = {(17-wkl)*rozm+szer*rozm*i, (19-wkl)*rozm+szer*rozm*i, (19-wyp)*rozm+szer*rozm*i, (17-wyp)*rozm+szer*rozm*i, (15-wyp)*rozm+szer*rozm*i, (15-wkl)*rozm+szer*rozm*i};
           int cy [] = {18*rozm, 20*rozm, 29*rozm, 31*rozm, 29*rozm, 20*rozm};
           a.setColor(colC);
           a.fillPolygon(cx,cy,npoints);
                     
           int dx [] = {(3+wyp)*rozm+szer*rozm*i,(5+wyp)*rozm+szer*rozm*i,(14-wyp)*rozm+szer*rozm*i,(16-wyp)*rozm+szer*rozm*i,(14-wyp)*rozm+szer*rozm*i,(5+wyp)*rozm+szer*rozm*i};
           int dy [] = {32*rozm,30*rozm,30*rozm,32*rozm,34*rozm,34*rozm};
           a.setColor(colD);
           a.fillPolygon(dx,dy,npoints);
           
           int ex [] = {(2+wkl)*rozm+szer*rozm*i, (4+wkl)*rozm+szer*rozm*i, (4+wyp)*rozm+szer*rozm*i, (2+wyp)*rozm+szer*rozm*i, (0+wyp)*rozm+szer*rozm*i, (0+wkl)*rozm+szer*rozm*i};
           int ey [] = {18*rozm, 20*rozm, 29*rozm, 31*rozm, 29*rozm, 20*rozm};
           a.setColor(colE);
           a.fillPolygon(ex,ey,npoints);
       
           int fx [] = {(2+wyp)*rozm+szer*rozm*i, (4+wyp)*rozm+szer*rozm*i, (4+wkl)*rozm+szer*rozm*i, (2+wkl)*rozm+szer*rozm*i, (0+wkl)*rozm+szer*rozm*i, (0+wyp)*rozm+szer*rozm*i};
           int fy [] = {3*rozm, 5*rozm, 14*rozm, 16*rozm, 14*rozm, 5*rozm};
           a.setColor(colF);
           a.fillPolygon(fx,fy,npoints);
                    
           int gx [] = {(3+wkl)*rozm+szer*rozm*i, (5+wkl)*rozm+szer*rozm*i, (14-wkl)*rozm+szer*rozm*i, (16-wkl)*rozm+szer*rozm*i, (14-wkl)*rozm+szer*rozm*i, (5+wkl)*rozm+szer*rozm*i};
           ...
           int kropkax [] = {18*rozm+szer*rozm*i, 21*rozm+szer*rozm*i, 21*rozm+szer*rozm*i, 18*rozm+szer*rozm*i};
